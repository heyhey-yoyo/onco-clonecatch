(() => {
  ...
  function updateMethods(fingerprint) {
    ...
    $("#methodsText").value = `CloneCatch 合成空间取样实验\n\n` +
      `使用 CloneCatch v1.1 科研模拟沙盒生成二维合成肿瘤网格（随机种子 ${config.seed}）。模型包含 ${config.cloneCount} 个空间克隆类别，稀有克隆目标总体占比为 ${config.rarePct.toFixed(1)}%，空间聚集度参数为 ${config.clustering}/100，空间结构为“${ARCH_NAMES[config.architecture]}”。` +
