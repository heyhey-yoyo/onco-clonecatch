...
- 视觉验收以正文 16px、操作与状态标签不小于 12px 为基线；画布注释保持高对比度，并在 1440px 桌面与 390px 手机视口检查整体横向溢出
...
主题只在一处定义顶层根变量；画布颜色由 CSS 变量读取，抽样与肿瘤模型逻辑不参与视觉清理。

统一品牌页眉、字号与浅色说明文字对比度，保留克隆分类颜色。浏览器验证虚拟取样、PNG 文件签名及三档尺寸/DPR；Firefox 下载实测受本机运行时限制，不能以 Chromium 结果代替。
